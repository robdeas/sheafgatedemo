...
var (
	AppName               = "sheafgatedemo" // overridden at build time
	SheafGateLauncherUUID = ""              // must be set at build time — fails fast if missing
	Version               = "dev"
	BuildTime             = "unknown"
	GitCommit             = "unknown"
)
...
func main() {
	if SheafGateLauncherUUID == "" {
		log.Fatal("[sheafgatelauncher] SheafGateLauncherUUID not set — build with -ldflags -X main.SheafGateLauncherUUID=...")
	}
	...
	url := fmt.Sprintf(
		...
		port, SheafGateLauncherUUID, password,
	)
...
func shutdownEngine() {
	...
	url := fmt.Sprintf(
		...
		enginePort, SheafGateLauncherUUID,
	)
	...
	if err := engineCmd.Process.Kill(); err == nil {
		log.Printf("[sheafgatelauncher] engine process killed")
	}
}
...
func launchBun(password string, port int, cfg LauncherConfig) (int, error) {
	...
	for _, e := range cmd.Env {
		if strings.HasPrefix(e, "PORT=") ||
			...
			log.Printf("[sheafgatelauncher] env: %s", e)
		}
	}
